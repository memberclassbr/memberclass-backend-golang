...
func TestUserInformationsHandler_GetUserInformations_Success(t *testing.T) {
	...
	users := []response.UserInformation{
		{
			UserID:     "user-123",
			Email:      "test@example.com",
			IsPaid:     true,
			Deliveries: []response.DeliveryInfo{},
	...
	var result response.UserInformationsResponse
...
func TestUserInformationsHandler_GetUserInformations_UserNotFound(t *testing.T) {
	...
	mockUseCase.On("GetUserInformations", mock.Anything, mock.Anything, tenantID).Return(nil, usecases.ErrUserNotFoundOrNotInTenantForInformations)
...
func TestUserInformationsHandler_GetUserInformations_EmptyResults(t *testing.T) {
	...
	emptyResponse := &response.UserInformationsResponse{
		Users: []response.UserInformation{},
		Pagination: response.UserInformationsPagination{
			Page:            1,
			TotalPages:      0,
