...
func (m *MockPdfProcessorUseCase) CreateOrUpdatePDFAsset(ctx context.Context, lessonID, pdfURL string) (*lessonsentities.LessonPDFAsset, error) {
...
func (m *MockPdfProcessorUseCase) GetLessonWithPDFAsset(ctx context.Context, lessonID string) (*lessonsentities.Lesson, error) {
	args := m.Called(ctx, lessonID)
	return args.Get(0).(*lessonsentities.Lesson), args.Error(1)
}

func (m *MockPdfProcessorUseCase) GetPDFPagesByAssetID(ctx context.Context, assetID string) ([]*lessonsentities.LessonPDFPage, error) {
	args := m.Called(ctx, assetID)
	return args.Get(0).([]*lessonsentities.LessonPDFPage), args.Error(1)
}
...
func TestGetLessonsPage_Success(t *testing.T) {
	...
	lesson := &lessonsentities.Lesson{
		ID: &lessonID,
		PDFAsset: &lessonsentities.LessonPDFAsset{
			ID:         assetID,
			LessonID:   lessonID,
			Status:     "done",
			TotalPages: intPtr(3),
	...
	// Mock PDF pages
	pages := []*lessonsentities.LessonPDFPage{
		{
			ID:         "page1",
			AssetID:    assetID,
			PageNumber: 1,
	...
	var response lessondto.LessonPDFPagesResponse
...
func TestGetLessonsPage_LessonNotFound(t *testing.T) {
	...
	// Setup expectations
	mockUseCase.On("GetLessonWithPDFAsset", mock.Anything, lessonID).Return((*lessonsentities.Lesson)(nil), &memberclasserrors.MemberClassError{
		Code:    404,
