...
type UserInformationsHandler struct {
	useCase ports.UserInformationsUseCase
	logger  ports.Logger
}

func NewUserInformationsHandler(useCase ports.UserInformationsUseCase, logger ports.Logger) *UserInformationsHandler {
...
func (h *UserInformationsHandler) handleUseCaseError(w http.ResponseWriter, err error) {
	if errors.Is(err, usecases.ErrUserNotFoundOrNotInTenantForInformations) {
