...
type AILessonHandler struct {
	useCase ai2.AILessonUseCase
	logger  ports.Logger
}

func NewAILessonHandler(useCase ai2.AILessonUseCase, logger ports.Logger) *AILessonHandler {
...
func (h *AILessonHandler) GetLessons(w http.ResponseWriter, r *http.Request) {
	...
	req := ai.ParseGetAILessonsRequest(tenantID, onlyUnprocessed)
