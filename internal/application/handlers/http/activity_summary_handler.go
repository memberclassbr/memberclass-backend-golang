...
type ActivitySummaryHandler struct {
	useCase ports.ActivitySummaryUseCase
	logger  ports.Logger
}

func NewActivitySummaryHandler(useCase ports.ActivitySummaryUseCase, logger ports.Logger) *ActivitySummaryHandler {
...
func (h *ActivitySummaryHandler) GetActivitySummary(w http.ResponseWriter, r *http.Request) {
	...
	req, err := request.ParseActivitySummaryRequest(r.URL.Query())
	if err != nil {
