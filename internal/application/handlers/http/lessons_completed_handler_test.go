...
func TestLessonsCompletedHandler_GetLessonsCompleted_Success(t *testing.T) {
	...
	responseData := &response.LessonsCompletedResponse{
		OK: true,
		Data: response.LessonsCompletedData{
			CompletedLessons: []response.CompletedLesson{
				{
	...
	var result response.LessonsCompletedResponse
...
func TestLessonsCompletedHandler_GetLessonsCompleted_UserNotFound(t *testing.T) {
	...
	mockUseCase.On("GetLessonsCompleted", mock.Anything, mock.Anything, tenantID).Return(nil, usecases.ErrUserNotFoundOrNotInTenantForLessons)
