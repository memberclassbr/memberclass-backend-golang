...
type StudentReportHandler struct {
	useCase ports.StudentReportUseCase
	logger  ports.Logger
}

func NewStudentReportHandler(useCase ports.StudentReportUseCase, logger ports.Logger) *StudentReportHandler {
...
func (h *StudentReportHandler) GetStudentReport(w http.ResponseWriter, r *http.Request) {
	...
	parsedReq, err := request.ParseStudentReportRequest(r.URL.Query())
	if err != nil {
