...
type LessonsCompletedHandler struct {
	useCase ports.LessonsCompletedUseCase
	logger  ports.Logger
}

func NewLessonsCompletedHandler(useCase ports.LessonsCompletedUseCase, logger ports.Logger) *LessonsCompletedHandler {
...
func (h *LessonsCompletedHandler) GetLessonsCompleted(w http.ResponseWriter, r *http.Request) {
	...
	req, err := request.ParseLessonsCompletedRequest(r.URL.Query())
	if err != nil {
...
func (h *LessonsCompletedHandler) handleUseCaseError(w http.ResponseWriter, err error) {
	if errors.Is(err, usecases.ErrUserNotFoundOrNotInTenantForLessons) {
