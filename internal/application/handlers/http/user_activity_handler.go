...
type UserActivityHandler struct {
	useCase ports.UserActivityUseCase
	logger  ports.Logger
}

func NewUserActivityHandler(useCase ports.UserActivityUseCase, logger ports.Logger) *UserActivityHandler {
...
func (h *UserActivityHandler) handleUseCaseError(w http.ResponseWriter, err error) {
	if errors.Is(err, usecases.ErrUserNotFoundOrNotInTenant) {
