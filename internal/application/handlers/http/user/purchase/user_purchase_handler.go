...
type UserPurchaseHandler struct {
	useCase user.UserPurchaseUseCase
	logger  ports.Logger
}

func NewUserPurchaseHandler(useCase user.UserPurchaseUseCase, logger ports.Logger) *UserPurchaseHandler {
...
func (h *UserPurchaseHandler) handleUseCaseError(w http.ResponseWriter, err error) {
	if errors.Is(err, user2.ErrUserNotFoundOrNotInTenantForPurchases) {
