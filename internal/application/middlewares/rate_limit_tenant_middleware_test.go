...
func TestRateLimitTenantMiddleware_LimitByTenant(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			tenant: &entities.Tenant{ID: "tenant-123", Name: "Test Tenant"},
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.tenant != nil {
				...
				if tt.checkLimitError == nil {
					...
					mockRateLimiter.EXPECT().CheckLimit(mock.Anything, tt.tenant.ID, tt.endpoint).Return(false, ports.RateLimitInfo{}, tt.checkLimitError)
