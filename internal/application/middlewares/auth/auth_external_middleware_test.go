...
func (m *MockApiTokenUseCase) ValidateToken(ctx context.Context, token string) (*tenantentities.Tenant, error) {
	args := m.Called(ctx, token)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
	return args.Get(0).(*tenantentities.Tenant), args.Error(1)
}
...
func TestAuthExternalMiddleware_Authenticate_Success(t *testing.T) {
	...
	var contextTenant *tenantentities.Tenant
