...
type SocialCommentUseCase struct {
	...
	socialCommentRepo ports.SocialCommentRepository
	topicRepo         ports.TopicRepository
}

func NewSocialCommentUseCase(logger ports.Logger, userRepo ports.UserRepository, socialCommentRepo ports.SocialCommentRepository, topicRepo ports.TopicRepository) ports.SocialCommentUseCase {
...
func (uc *SocialCommentUseCase) CreateOrUpdatePost(ctx context.Context, req request.CreateSocialCommentRequest, tenantID string) (*response.SocialCommentResponse, error) {
...
func (uc *SocialCommentUseCase) updatePost(ctx context.Context, req request.CreateSocialCommentRequest, tenantID string) (*response.SocialCommentResponse, error) {
	...
	return &response.SocialCommentResponse{
		OK: true,
		ID: req.PostID,
	}, nil
}

func (uc *SocialCommentUseCase) createPost(ctx context.Context, req request.CreateSocialCommentRequest, tenantID string) (*response.SocialCommentResponse, error) {
	isOwner, err := uc.userRepo.IsUserOwner(ctx, req.UserID, tenantID)
	if err != nil {
		return nil, err
	}
