...
type StudentReportUseCase struct {
	logger           ports.Logger
	studentReportRepo ports.StudentReportRepository
	cache            ports.Cache
}

func NewStudentReportUseCase(logger ports.Logger, studentReportRepo ports.StudentReportRepository, cache ports.Cache) ports.StudentReportUseCase {
...
func (uc *StudentReportUseCase) GetStudentReport(ctx context.Context, req request.GetStudentReportRequest, tenantID string) (*response.StudentReportResponse, error) {
	...
	if err == nil && cachedData != "" {
		var cachedResponse response.StudentReportResponse
	...
	pagination := response.StudentReportPagination{
		Page:        req.Page,
		Limit:       req.Limit,
	...
	responseData := &response.StudentReportResponse{
		Alunos:     students,
...
func (uc *StudentReportUseCase) buildCacheKey(tenantID string, req request.GetStudentReportRequest) string {
