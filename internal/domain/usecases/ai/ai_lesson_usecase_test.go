...
func TestAILessonUseCase_UpdateTranscriptionStatus(t *testing.T) {
	...
	tests := []struct {
		...
		validateResult func(*testing.T, *lesson2.LessonTranscriptionResponse)
	}{
		{
			name:     "should return error when lessonID is empty",
			lessonID: "",
			req:      lesson.UpdateLessonTranscriptionRequest{TranscriptionCompleted: true},
			mockSetup: func(*mocks.MockLessonRepository, *mocks.MockLogger) {
			},
			expectError: true,
			expectedError: &memberclasserrors.MemberClassError{
		...
		},
		{
			...
			expectError: false,
			validateResult: func(t *testing.T, result *lesson2.LessonTranscriptionResponse) {
		...
		},
		{
			...
			lessonID: lessonID,
			req:      lesson.UpdateLessonTranscriptionRequest{TranscriptionCompleted: false},
			mockSetup: func(mockLessonRepo *mocks.MockLessonRepository, mockLogger *mocks.MockLogger) {
				mockLessonRepo.EXPECT().GetByIDWithTenant(mock.Anything, lessonID).Return(
					&lessons.Lesson{
						ID:                     &lessonID,
...
func TestAILessonUseCase_GetLessons(t *testing.T) {
	...
	tests := []struct {
		...
		expectError    bool
		expectedError  *memberclasserrors.MemberClassError
		validateResult func(*testing.T, *ai2.AILessonsResponse)
	}{
		{
		...
		},
		{
			...
			mockSetup: func(mockLessonRepo *mocks.MockLessonRepository, mockTenantRepo *mocks.MockTenantRepository, mockLogger *mocks.MockLogger) {
				mockTenantRepo.EXPECT().FindByID(tenantID).Return(&tenant.Tenant{
					ID:        tenantID,
					AIEnabled: false,
				}, nil)
			},
		...
		},
		{
			...
			mockSetup: func(mockLessonRepo *mocks.MockLessonRepository, mockTenantRepo *mocks.MockTenantRepository, mockLogger *mocks.MockLogger) {
				...
				mockLessonRepo.EXPECT().GetLessonsWithHierarchyByTenant(mock.Anything, tenantID, false).Return([]lesson3.AILessonWithHierarchy{
					{
						ID:                     "lesson-1",
						Name:                   "Lesson 1",
						Slug:                   "lesson-1",
			...
			expectError: false,
			validateResult: func(t *testing.T, result *ai2.AILessonsResponse) {
		...
		},
		{
			...
			mockSetup: func(mockLessonRepo *mocks.MockLessonRepository, mockTenantRepo *mocks.MockTenantRepository, mockLogger *mocks.MockLogger) {
				...
				mockLessonRepo.EXPECT().GetLessonsWithHierarchyByTenant(mock.Anything, tenantID, true).Return([]lesson3.AILessonWithHierarchy{
					{
						ID:                     "lesson-2",
						Name:                   "Lesson 2",
						Slug:                   "lesson-2",
		...
		},
		{
			...
			mockSetup: func(mockLessonRepo *mocks.MockLessonRepository, mockTenantRepo *mocks.MockTenantRepository, mockLogger *mocks.MockLogger) {
				...
				mockLessonRepo.EXPECT().GetLessonsWithHierarchyByTenant(mock.Anything, tenantID, false).Return([]lesson3.AILessonWithHierarchy{}, nil)
			},
			expectError: false,
			validateResult: func(t *testing.T, result *ai2.AILessonsResponse) {
				assert.Equal(t, tenantID, result.TenantID)
