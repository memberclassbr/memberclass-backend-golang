...
func TestAITenantUseCase_GetTenantsWithAIEnabled(t *testing.T) {
	...
	tests := []struct {
		...
		validateResult func(*testing.T, *ai.AITenantsResponse)
	}{
		{
		...
		},
		{
			...
			mockSetup: func(mockTenantRepo *mocks.MockTenantRepository, mockLogger *mocks.MockLogger) {
				mockTenantRepo.EXPECT().FindAllWithAIEnabled(mock.Anything).Return([]*tenant.Tenant{}, nil)
			},
			expectError: false,
			validateResult: func(t *testing.T, result *ai.AITenantsResponse) {
		...
		},
		{
			...
			mockSetup: func(mockTenantRepo *mocks.MockTenantRepository, mockLogger *mocks.MockLogger) {
				mockTenantRepo.EXPECT().FindAllWithAIEnabled(mock.Anything).Return([]*tenant.Tenant{
					{
						ID:                 "tenant-1",
						Name:               "Tenant 1",
						AIEnabled:          true,
