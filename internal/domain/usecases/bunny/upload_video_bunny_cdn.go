...
type UploadVideoBunnyCdnUseCase struct {
	bunnyService bunny.BunnyService
	log          ports.Logger
}

func (u *UploadVideoBunnyCdnUseCase) Execute(ctx context.Context, bunnyParams dto.BunnyParametersAccess, fileBytes []byte, title string) (*response.UploadVideoResponse, error) {
...
func NewUploadVideoBunnyCdnUseCase(bunnyService bunny.BunnyService, log ports.Logger) bunny.UploadVideoBunnyCdnUseCase {
