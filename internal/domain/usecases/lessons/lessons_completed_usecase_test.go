...
func TestLessonsCompletedUseCase_GetLessonsCompleted_Success(t *testing.T) {
	...
	req := lesson.GetLessonsCompletedRequest{
		Email: email,
		Page:  1,
		Limit: 10,
	}
	...
	lessons := []lesson2.CompletedLesson{
		{
	...
	assert.Equal(t, int64(2), result.Data.Pagination.TotalCount)
...
func TestLessonsCompletedUseCase_GetLessonsCompleted_Pagination(t *testing.T) {
	...
	assert.Equal(t, int64(25), result.Data.Pagination.TotalCount)
...
func TestLessonsCompletedUseCase_GetLessonsCompleted_EmptyResults(t *testing.T) {
	...
	mockUserRepo.On("FindByEmail", email).Return(user, nil)
	mockUserRepo.On("BelongsToTenant", userID, tenantID).Return(true, nil)
	mockLessonRepo.On("FindCompletedLessonsByEmail", mock.Anything, userID, tenantID, mock.AnythingOfType("time.Time"), mock.AnythingOfType("time.Time"), "", 1, 10).Return([]lesson2.CompletedLesson{}, int64(0), nil)
	...
	assert.True(t, result.OK)
	assert.Equal(t, 0, len(result.Data.CompletedLessons))
	assert.Equal(t, int64(0), result.Data.Pagination.TotalCount)
