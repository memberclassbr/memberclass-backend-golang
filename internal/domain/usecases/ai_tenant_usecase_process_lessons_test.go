...
func TestAITenantUseCase_ProcessLessonsTenant(t *testing.T) {
	tests := []struct {
		...
		validateResult func(*testing.T, *response.ProcessLessonsTenantResponse)
	}{
		{
		...
		},
		{
			...
			mockSetup: func(mockTenantRepo *mocks.MockTenantRepository, mockAILessonUseCase *mocks.MockAILessonUseCase, mockLogger *mocks.MockLogger, server *httptest.Server) {
				...
				mockAILessonUseCase.EXPECT().GetLessons(mock.Anything, request.GetAILessonsRequest{
					TenantID:        "tenant-123",
			...
			expectError: false,
			validateResult: func(t *testing.T, result *response.ProcessLessonsTenantResponse) {
...
func TestAITenantUseCase_ProcessLessonsTenant_APIError(t *testing.T) {
	...
	result, err := useCase.ProcessLessonsTenant(context.Background(), request.ProcessLessonsTenantRequest{
		TenantID: "tenant-123",
	})
