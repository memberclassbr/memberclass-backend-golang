...
func (m *MockCommentRepository) Update(ctx context.Context, commentID, answer string, published bool) (*comment2.Comment, error) {
	args := m.Called(ctx, commentID, answer, published)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
	return args.Get(0).(*comment2.Comment), args.Error(1)
}

func (m *MockCommentRepository) FindByIDAndTenant(ctx context.Context, commentID, tenantID string) (*comment2.Comment, error) {
...
func (m *MockCommentRepository) FindByIDAndTenantWithDetails(ctx context.Context, commentID, tenantID string) (*comment.CommentResponse, error) {
	args := m.Called(ctx, commentID, tenantID)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
...
func (m *MockCommentRepository) FindAllByTenant(ctx context.Context, tenantID string, req *comments.GetCommentsRequest) ([]*comment.CommentResponse, int64, error) {
	args := m.Called(ctx, tenantID, req)
	if args.Get(0) == nil {
		return nil, args.Get(1).(int64), args.Error(2)
	}
	return args.Get(0).([]*comment.CommentResponse), args.Get(1).(int64), args.Error(2)
}
...
func TestCommentUseCase_UpdateAnswer_Success(t *testing.T) {
	...
	existingComment := &comment2.Comment{
		ID:       &commentID,
		Question: &question,
		Answer:   nil,
	}

	updatedComment := &comment2.Comment{
		ID:        &commentID,
		Question:  &question,
		Answer:    &answer,
		Published: published,
