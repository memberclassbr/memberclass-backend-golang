...
func (m *MockUserRepository) FindPurchasesByUserAndTenant(ctx context.Context, userID string, tenantID string, purchaseTypes []string, page int, limit int) ([]response.UserPurchaseData, int64, error) {
	args := m.Called(ctx, userID, tenantID, purchaseTypes, page, limit)
	if args.Get(0) == nil {
		return nil, args.Get(1).(int64), args.Error(2)
	}
	return args.Get(0).([]response.UserPurchaseData), args.Get(1).(int64), args.Error(2)
}
